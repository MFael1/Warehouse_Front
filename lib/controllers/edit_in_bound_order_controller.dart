...
class Editinordercontroller extends GetxController {
  final GlobalKey<FormState> editinordercontroller = GlobalKey<FormState>();
  final API api = API();
  ...
  var items = <InorderItemEdit>[].obs;
  var itemsList = <InorderItemEdit>[].obs;
  ...
  void checkPurchaseOrder() {
    if (editinordercontroller.currentState == null) {
      SnackbarUtils.showCustomSnackbar(
        title: 'Error',
    ...
    final isValid = editinordercontroller.currentState!.validate();
    if (!isValid) return;

    editinordercontroller.currentState!.save();
    submitPurchaseOrder();
  }

  void addItem() {
    items.add(InorderItemEdit(
      itemId: 0,
      itemName: '',
      orderedQuantity: 0,
      deliveredQuantity: 0,
