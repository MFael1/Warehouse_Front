...
class DisplayINorder extends GetxController {
  final GlobalKey<FormState> displayINorder = GlobalKey<FormState>();
  ...
  void checkAddPurchaseOrder() {
    if (displayINorder.currentState == null) {
      print('Error: purchaseOrderFormKey.currentState is null');
      Get.snackbar('Error', 'Form state is null');
      return;
    }
    final isValid = displayINorder.currentState!.validate();
    if (!isValid) {
      print('Form is not valid');
      return;
    }
    print('Form is valid');
    displayINorder.currentState!.save();
    print('Form saved');
  ...
  //   const url =
  //       'https://api.example.com/purchase_orders'; // Replace with your API endpoint
  ...
  // try {
  // final response = await apiService.createPurchaseOrder(PurchaseOrder(
  ...
  //   supplier: Supplier(id: 1, name: "DEFAULT", contact: Contact(id: 1, phoneNumber: "0000000000")),  // Replace with actual data
